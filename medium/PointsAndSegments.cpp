...
std::vector<int> PointsAndSegments(const std::vector<std::vector<int>>& iPoints)
{
    ...
    std::vector<std::array<int, 3>> new_points(segments_count * 2 + points_count);
    int j = 0;
    for (int i = 0; i < segments_count; ++i)
    {
        new_points[j++] = {iPoints[i + 1].front(), +1, -1};
        new_points[j++] = {iPoints[i + 1].back(), -1, -1};
    }
    
    for (int i = 0; i < points_count; ++i)
    {
        new_points[j++] = {iPoints.back()[i], 0, i};
    }
    ...
    for (int i = 0, I = (int)new_points.size(); i < I; ++i)
    {
        ...
        if (new_points[i][1] == 0)
        {
            result_points[new_points[i][2]] = sum;
        }
    }
